...
mean_n_plot <- ggplot(summary_df, aes(x = CHROM, y = mean_N_VARIANTS, fill = TREAT)) +
  ...
  geom_errorbar(aes(ymin = mean_N_VARIANTS - se_N_VARIANTS, ymax = mean_N_VARIANTS + se_N_VARIANTS),
                position = position_dodge(width = 0.8), width = 0.2) +
  scale_fill_manual(values = col_pal, name = "Treatment") +
  labs(title = "Mean N_VARIANTS per Chromosome", x = "Chromosome", y = "Mean N_VARIANTS") +
  theme_minimal()
...
### For loop for statistics and visualizations
...
# Define chromosome order explicitly
chrom_levels <- as.character(1:10)
...
for (i in seq_along(df_names)) {
  ...
  chrom_summary <- df %>%
    ...
    mutate(
      dataset = label,
      CHROM = factor(CHROM, levels = chrom_levels)  # enforce order
    )
...
# Plot: Mean PI per CHROM by dataset
ggplot(summary_df, aes(x = CHROM, y = mean_PI, fill = dataset)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  labs(title = "Mean PI per Chromosome", y = "Mean PI", x = "Chromosome") +
  theme_minimal()
...
for (i in seq_along(df_names)) {
  ...
  p <- ggplot(df, aes(x = N_VARIANTS, y = PI)) +
    ...
    labs(title = paste("Correlation: PI vs N_VARIANTS —", label),
         x = "N_VARIANTS",
         y = "PI") +
    theme_minimal()

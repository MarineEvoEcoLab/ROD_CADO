...
The following was run on the command line to create a file containing sample names in 1 column and treatment names in another column
```{bash}
...
Tajima's D by chromosome
#modify chromosome label in data frame 
...
tajd.df.chrom <- tajd.df %>%
  mutate(CHROM = str_replace_all(CHROM, replacements))
head(tajd.df.chrom)
```
Plot Tajima's D by chromosome
```{r}

tajd.df.chrom <- tajd.df.chrom %>%
  mutate(CHROM = factor(CHROM, levels = as.character(1:10))) # Ensure factor order

cp <-ggplot(tajd.df.chrom, aes(x=TAJD,y = CHROM))+
  geom_point(aes(color=CHROM,fill= CHROM), position="jitter")+
  #geom_violin(aes(color=CHROM,fill= CHROM)) +
  #stat_summary(fun=mean, geom="point", shape=23, size=4)+
  #geom_dotplot(binaxis='y', stackdir='center', dotsize=0.005)+
  geom_boxplot(aes(fill=CHROM), width=0.75,outlier.shape = 23, outlier.color = "black")+
  stat_summary(fun=mean, geom="point", shape=23, size=2)+
  #scale_fill_manual(values=col_pal)+
  #scale_color_manual(values=col_pal)+
